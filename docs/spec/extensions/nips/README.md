...
|---|---|---|
| `heterodyne:0.6.0#core-identity-discovery` | Source-neutral Nostr repository discovery | Active-key-signed hints for persona-owned repositories, serving nodes, and relays without making any carrier authoritative. |
| `heterodyne:0.6.0#assurance-reciprocal-enrollment` | Optional Nostr account continuity | Reciprocal active-key and recovery-authority enrollment, explicit succession, and non-aliasing continuity as an opt-in layer. |
| `heterodyne:0.6.0#comms-marmot-event-repository` | Marmot archive routing through Radicle | Exact-byte encrypted-event archiving and routing authorization that preserves upstream Marmot semantics. |
