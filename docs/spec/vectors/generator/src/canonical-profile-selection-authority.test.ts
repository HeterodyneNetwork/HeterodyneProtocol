...
describe("canonical Core profile selection authority", () => {
  ...
  it("BLUE TEAM VALIDATION: synthetic/local rejects constructor configuration that can relabel a genuine binding", async () => {
    const canonical = await loadCanonical();
    const malicious = {
      ...config(),
      inspect_repository_candidate: () => ({
        candidate_event_id: "00".repeat(32),
        expected: {},
        binding: {},
      }),
    } as unknown as CanonicalProfileSelectionAuthorityConfig;

    expect(() => canonical.createCanonicalProfileSelectionAuthority?.(malicious))
      .toThrow("canonical-profile-authority-invalid");
  });
