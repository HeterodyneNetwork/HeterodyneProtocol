...
export function createPersonaSuccessionProof(
  ...
  const message = personaSuccessionPayload(candidate);
...
function personaSuccessionPayload(candidate: ContinuityManifestBody | ContinuityManifest): string {
  return `heterodyne-oidc-issuer-successor-v1\0${continuitySuccessorDigest(candidate as ContinuityManifest)}`;
}
