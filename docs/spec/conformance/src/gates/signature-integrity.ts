...
export function findSignatureIntegrityFailures(corpus: ArtifactCorpus): string[] {
  const failures = new Set<string>();

  for (const { value: vector } of corpus.vectors) {
    for (const check of vector.conformance_checks ?? []) {
      if (
        check.expected_terminal_stage !== "signature"
        && checkCoreSignedEvent(vector, check).terminalStage === "signature"
      ) {
