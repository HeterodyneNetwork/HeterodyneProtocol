  ...
  for (let anchorLine = 0; anchorLine < lines.length; anchorLine += 1) {
    ...
    sections.push({
      ...
      item: {
        ...
        owner, heading: heading[2], source_path: path, source_digest: sha256(bounded), evidence_kind: "normative_spec",
      },
    });
  }
  return sections;
...
function loadVectors(repo, ref) {
  ...
  if (!Array.isArray(snapshot.artifacts)) throw new Error(`malformed snapshot artifacts in ${resolved}:${SNAPSHOT}`);
  if (typeof snapshot.source_commit !== "string") throw new Error("snapshot source_commit is missing");
  const seenPaths = new Set();
  for (const artifact of snapshot.artifacts) {
    if (typeof artifact?.path !== "string" || !/^[0-9a-f]{64}$/.test(artifact?.sha256 ?? "")) throw new Error(`malformed snapshot artifact in ${resolved}:${SNAPSHOT}`);
    if (seenPaths.has(artifact.path)) throw new Error(`duplicate snapshot artifact: ${artifact.path}`);
    seenPaths.add(artifact.path);
  }
  if (!seenPaths.has(COVERAGE)) throw new Error(`snapshot does not declare ${COVERAGE}`);
  const artifactRead = readInputs(repo, resolved, { paths: [...seenPaths] });
  for (const artifact of snapshot.artifacts) {
...
function createGraph(sourceContents, vectorData) {
  ...
  for (const section of sections) for (const artifact of referencedArtifacts) {
    if (artifact.keys.some((key) => section.content.includes(key))) edges.push({ from: section.item.semantic_id, to: artifact.value.semantic_id, relation: artifact.relation, source_path: section.item.source_path });
  }
  if (vectorData) {
    ...
    for (const [path, content] of vectorData.contents) {
      if (path === SNAPSHOT || path === COVERAGE || !path.endsWith(".json")) continue;
      const value = parseJson(content, `${vectorData.resolved_ref}:${path}`);
      if (typeof value.vector_id !== "string") continue;
      if (!/^[23](?:\.|$)/.test(String(value.vector_schema_version))) throw new Error(`unsupported vector schema: ${value.vector_schema_version}`);
    ...
    if (Number.isInteger(vectorData.snapshot.vector_count) && vectors.size !== vectorData.snapshot.vector_count) throw new Error(`snapshot vector_count mismatch: expected ${vectorData.snapshot.vector_count}, found ${vectors.size}`);
...
export async function buildProjection({ repo, lane, vectorRef, ref } = {}) {
  ...
  const receipt = {
    ...
    inputs, input_inventory_sha256: graphDigest(inputs.map(({ path }) => path)),
...
export function verifyReceipt(projection) {
  ...
  try {
    ...
    const inventory = graphDigest(currentInputs.map(({ path }) => path));
